...
const examples: ReadonlyArray<[string, string?]> = [
  ...
  [
    `${APP_NAME} analyze -f ./ts-for-gir-report.json --severity error critical`,
  ...
  [
    `${APP_NAME} analyze -f ./ts-for-gir-report.json --namespace GLib --top 5`,
    "Show top 5 problems in GLib namespace",
  ],
  [
...
const filterProblems = (problems: ProblemEntry[], args: AnalyzeCommandArgs): ProblemEntry[] => {
  ...
  if (args.namespace?.length) {
    filtered = filtered.filter((p) =>
      args.namespace?.some(
        (ns) => p.location?.includes(ns) || p.module?.includes(ns) || p.metadata?.namespace === ns,
      ),
    );
  }
...
const handler = async (args: AnalyzeCommandArgs): Promise<void> => {
  ...
  try {
    ...
    const hasFilters = Boolean(
      args.severity || args.category || args.namespace || args.type || args.search,
    );
