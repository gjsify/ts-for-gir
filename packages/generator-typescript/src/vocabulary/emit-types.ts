 ...
 *
 * ${surface.chains.size} instantiable GTypes (of which ${surface.widgets.length} concrete widgets${surface.childHolders.length > 0 ? ` and ${surface.childHolders.length} child holders` : ""}), ${emitted.length} declarations${inlined.length > 0 ? ` (${inlined.length} inlined from a namespace whose vocabulary does not emit them)` : ""}, ${surface.enums.size} enum nick unions, ${slotTotal} slot candidates.
 *
