...
function updateDisplay() {
	...
	if (calcVal === "") label.set_markup("<span size='30000'>0</span>");
}
...
function pressedEquals() {
	...
	// biome-ignore lint/security/noGlobalEval: Calculator needs to evaluate math expressions
	calcVal = eval(calcVal as string) as number;
	// Avoid ridiculous amounts of precision from toString.
	if (Number(calcVal) === Math.floor(Number(calcVal))) calcVal = Math.floor(calcVal);
	// bizarrely gjs loses str.toFixed() somehow?!
	else calcVal = Math.floor(calcVal * 10000) / 10000;
	label.set_markup(`<span size='30000'>${calcVal}</span>`);
}
...
function packButtons(buttons: Gtk.Button[], vbox: Gtk.VBox) {
	...
	for (let i = 0; i <= 4; i++) hbox.pack_start(buttons[i], true, true, 1);
}
