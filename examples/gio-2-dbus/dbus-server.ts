...
class Service {
	...
	get ReadWriteProperty() {
		if (this._readWriteProperty === undefined) return false;
	...
	set ReadWriteProperty(value) {
		if (this.ReadWriteProperty !== value) {
			...
			this.dbus.emit_property_changed("ReadWriteProperty", new GLib.Variant("b", value));
		}
	}
	...
	// Signals
	emitTestSignal() {
		this.dbus.emit_signal("TestSignal", new GLib.Variant("(sb)", ["string", false]));
	}
}
