...
import { buildWidgetVocabulary, emitVocabularyData, emitVocabularyTypes } from "../vocabulary/index.ts";
...
export class ModuleExporter {
	...
	private async exportWidgetVocabulary(girModule: GirModule): Promise<void> {
		...
		await this.moduleTemplateProcessor.write(emitVocabularyData(surface), this.config.outdir, `${name}-vocabulary.js`);
