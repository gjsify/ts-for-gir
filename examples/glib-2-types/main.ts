...
function testTimestampHandling(): void {
  ...
  try {
    ...
    console.log(
      `  Date: ${futureDate.get_day()}/${futureDate.get_month()}/${futureDate.get_year()}`,
    );
  } catch (error) {
    console.log(`  Error: ${error}`);
  }
...
function testUnderscoreInit() {
  const errorEnum = new GLib.BookmarkFileError({
    message: "foo",
    code: GLib.BookmarkFileError.WRITE,
  });
  try {
    // @ts-expect-error
    errorEnum._init();
...
function testGErrorGType() {
  if (GLib.BookmarkFileError.$gtype !== GLib.Error.$gtype)
    throw new Error("GError GTypes should be equal");
  // @ts-expect-error
...
function testEnumGType() {
  ...
  if (GLib.NormalizeMode.$gtype === undefined)
    throw new Error("Registered enum should have a $gtype property");
}

function testEnumNotIntrospectable() {
  // @ts-expect-error
  if (GLib.ThreadPriority)
    throw new Error("Non-introspectable enum shouldn't be in type definitions");
}
...
function displaySummary(): void {
  ...
  console.log(
    "✓ DestroyNotify params stripped from idle_add/timeout_add — GJS handles destroy internally",
  );
