...
type AnySignalCallback<T extends GObject.Object = GObject.Object> = GObject.SignalCallback<
  T,
  (...args: any[]) => any
>;
...
type SignalProps<T extends GObject.Object> = T extends { $signals: infer S }
  ? {
      ...
      [K in keyof S as `on${Capitalize<KebabToCamel<K & string>>}`]?: S[K] extends (
        ...args: any[]
      ) => any
...
const createElement = {
  ...
  ApplicationWindow: (
    props: ElementProps<Gtk.ApplicationWindow> &
      Partial<Gtk.ApplicationWindow.ConstructorProps> = {},
  ) => {
    const { ref, children, ...allProps } = props;
