...
import { analyzeOptions, appName } from "../config.ts";
...
const examples: ReadonlyArray<[string, string?]> = [
	[`${appName} analyze -f ./ts-for-gir-report.json`, "Show summary statistics of the report"],
	[`${appName} analyze -f ./ts-for-gir-report.json --summary`, "Show only summary statistics"],
	[`${appName} analyze -f ./ts-for-gir-report.json --severity error critical`, "Show only errors and critical issues"],
	[
		`${appName} analyze -f ./ts-for-gir-report.json --category type_resolution --detailed`,
		"Show detailed type resolution problems",
	],
	[`${appName} analyze -f ./ts-for-gir-report.json --namespace GLib --top 5`, "Show top 5 problems in GLib namespace"],
	[
		`${appName} analyze -f ./ts-for-gir-report.json --type time_t --export ./time_t_issues.json`,
		"Export all time_t related issues",
	],
	[
		`${appName} analyze -f ./ts-for-gir-report.json --search "Unable to resolve" --format csv`,
