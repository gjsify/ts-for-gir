 ...
 */
export function computeProblemStatistics(
  ...
  const unresolvedTypes: Record<
    string,
    { count: number; namespaces: Set<string>; sourceModules: Set<string> }
  > = {};
  ...
  for (const problem of problems) {
    ...
    if (problem.category === ProblemCategory.TYPE_RESOLUTION && problem.typeName) {
      if (!unresolvedTypes[problem.typeName]) {
        unresolvedTypes[problem.typeName] = {
          count: 0,
          namespaces: new Set(),
...
export function generateReportSummary(statistics: ReportStatistics): GenerationReport["summary"] {
  ...
  if (byCategory[ProblemCategory.GENERATION_FAILURE] > 0) {
    keyIssues.push(
      `${byCategory[ProblemCategory.GENERATION_FAILURE]} generation failures encountered`,
    );
...
export function groupProblemsByCategory(
  problems: ProblemEntry[],
): Record<ProblemCategory, ProblemEntry[]> {
