...
import { cleanupTestEnvironment, GIRS_DIR, MONOREPO_ROOT } from "../helpers.mjs";
...
describe("ts-for-gir GJS bundle E2E", { timeout: 10 * 60 * 1000 }, () => {
  ...
  before(() => {
    if (!existsSync(GJS_BIN)) {
      throw new Error(`GJS bundle not found at ${GJS_BIN}.\n` + "Run: yarn build:app:gjs");
    }
    if (!gjsAvailable()) {
      throw new Error(
        "gjs is not installed on this system. Install with: sudo apt-get install gjs",
      );
    }
  ...
  it("doc generates HTML documentation natively", () => {
    ...
    const result = runGjs(["doc", "GLib-2.0", "--girDirectories", GIRS_DIR, "--outdir", outdir]);
  ...
  it("json GLib-2.0 generates valid JSON natively", () => {
    ...
    const result = runGjs(["json", "GLib-2.0", "--girDirectories", GIRS_DIR, "--outdir", outdir]);
