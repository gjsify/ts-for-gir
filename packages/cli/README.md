...
Options:
  ...
  -t, -t       Specify what module code is generated.
...
Gjs now supports ES modules, which can be activated in `gjs` with its `gjs -m` flag. Using this in conjunction with `"module": "es6"` in `tsconfig.json` is generally more
...
Formats the generated .d.ts files with [ESLint](https://eslint.org/) and [Prettier](https://prettier.io/).

### verbose

Prints more warnings in the console.

### ignoreVersionConflicts

As soon as multiple versions of a `.gir` file are found (e.g. `Gtk-3.0.gir` and `Gtk-3.0.gir`) the user is asked which file should be used. This can be disabled with this option, then all versions will be loaded.

### print

Does not create files and instead outputs everything to the console.

### configName

The name of the [config file](#Config) which should be loaded.
...
Another advantage of TypeScript types generated by `ts-for-gir` is the generated documentation in TSDoc format. If this is not wanted, this option can be disabled with `noComments`.

### noDebugComments

We also generate some inline comments that are used for debugging. This can be disabled with this option.
...
The types described by GObject Introspection in GIR XML format are not 100% compatible with TypeScript. For example, methods can be overwritten by properties, which is not allowed in TypeScript. If this option is active, `ts-for-gir` tries to resolve these conflicts, but this leads to the problem that we provide types that do not exist like that. 