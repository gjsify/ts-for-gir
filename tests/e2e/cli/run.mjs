...
describe("ts-for-gir CLI E2E", { timeout: 10 * 60 * 1000 }, () => {
  ...
  it("generated types are valid TypeScript", () => {
    ...
    writeFileSync(
      testTsFile,
      "// Minimal type check — just verify generated files parse\nexport {};\n",
    );
  ...
  it("handles invalid module name gracefully", () => {
    const result = runCli(cliBin, projectWithPkg, [
      "generate",
      "NonExistentModule-99.0",
      "--girDirectories",
      GIRS_DIR,
      "--outdir",
      join(projectWithPkg, "types-invalid"),
    ]);
    assert.ok(
      result.exitCode !== 0 || result.stderr,
      "Expected non-zero exit or stderr for invalid module",
    );
  });
