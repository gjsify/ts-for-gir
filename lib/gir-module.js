...
const POD_TYPE_MAP_ARRAY = (runtime) => {
    return {
        guint8: runtime === 'gjs' ? 'Gjs.byteArray.ByteArray' : 'any',
        gint8: runtime === 'gjs' ? 'Gjs.byteArray.ByteArray' : 'any',
        gunichar: 'string',
    };
};
...
class GirModule {
    constructor(xml, runtime, buildtype) {
        ...
        this.conversation = new conversations_1.Conversation(runtime);
    ...
    typeLookup(e) {
        ...
        if (arr) {
            if (POD_TYPE_MAP_ARRAY(this.runtime)[type.$.name] != null) {
                return (POD_TYPE_MAP_ARRAY(this.runtime)[type.$.name] + nul);
            }
        }
    ...
    exportJs(outDir) {
        const templateProcessor = new template_processor_1.default({ name: this.name, version: this.version, runtime: this.runtime, buildtype: this.buildtype }, this.runtime);
        if (outDir) {
