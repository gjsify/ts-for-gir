...
describe("ts-for-gir create E2E", { timeout: 20 * 60 * 1000 }, () => {
  ...
  before(() => {
    ...
    assert.ok(
      existsSync(distTemplatesDir),
      `dist-templates/ missing in installed CLI at ${distTemplatesDir}`,
    );
    for (const t of TEMPLATES) {
      assert.ok(
        existsSync(join(distTemplatesDir, t)),
        `template "${t}" missing in dist-templates/`,
      );
    }
  });
  ...
  it("types-locally: install, generate, check, build", () => {
    ...
    const dts = readdirSync(join(projectDir, "@types"), { recursive: true }).filter(
  ...
  it("types-workspace: install, generate types, re-install, check sub-package", () => {
    ...
    assert.equal(
      buildTypes.status,
      0,
      `build:types failed: ${buildTypes.stderr}\n${buildTypes.stdout}`,
    );
    ...
    const adwPkg = JSON.parse(
      readFileSync(join(projectDir, "@girs", "adw-1", "package.json"), "utf8"),
    );
