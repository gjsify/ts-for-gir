...
const firstHandler = customAction.connect("value-changed", (source, value, label) => {
	...
	received.push(source === customAction ? `connect:${source.name}:${amount}:${text}` : "wrong source");
});
const afterHandler = customAction.connect_after("value-changed", (source, value, label) => {
	const amount: number = value;
	const text: string = label;
	received.push(source === customAction ? `after:${source.name}:${amount}:${text}` : "wrong source");
});
