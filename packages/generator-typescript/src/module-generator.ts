...
`;
		...
		def.push(
			...this.addGirDocComment(
				...
				[
					...
					...(tsFunction.throws ? [{ tagName: "throws", paramName: "", text: "GLib.Error" } as const] : []),
	...
	generateEnumMember(tsMember: GirEnumMember, indentCount = 1) {
		...
		desc.push(
			...this.addGirDocComment(tsMember.doc, this.namespace.getTsDocMetadataTags(tsMember.metadata), indentCount),
		);
	...
	generateConst(tsConst: IntrospectedConstant, indentCount = 0) {
		...
		const valueTag = tsConst.value ? [{ tagName: "default", paramName: "", text: tsConst.value } as const] : [];
