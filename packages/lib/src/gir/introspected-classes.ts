...
export class IntrospectedClass extends IntrospectedBaseClass {
    ...
    implementedProperties(potentialConflicts: IntrospectedBase<never>[] = []) {
        ...
        for (const implemented of resolution.implements()) {
            ...
            if (
                implementedOnParent.find((p) => p.identifier.equals(implemented.identifier))?.node?.generics?.length ===
                0
            )
                continue
        ...
        return {
            *[Symbol.iterator]() {
            ...
            extends(): ClassResolution | undefined {
                const resolved_parent = self.superType
                    ? resolveTypeIdentifier(self.namespace, self.superType)
                    : undefined
