...
app.connect("activate", () => {
  ...
  myButton.connect("hello", (_src, arg) => {
  ...
  entry.connect("notify", (_obj, pspec) => {
    if (pspec && pspec.get_name() === "text") {
      const text = entry.get_text();
      if (text.length > 0) {
        statusLabel.set_text(
          `Entry text changed via notify: "${text}" (${text.length} characters)`,
        );
      }
    }
  });
  ...
  toggleButton.connect("notify", (_obj, pspec) => {
  ...
  window.connect("notify", (_obj, pspec) => {
