...
function parseBundleMeta(raw: string): Record<string, unknown> {
	...
	try {
		...
		throw new Error(`--bundleMeta is not valid JSON: ${error instanceof Error ? error.message : String(error)}`);
	}
	...
	if (config.bundleMeta) {
		for (const reserved of ["name", "exports"]) {
			if (reserved in config.bundleMeta) {
				throw new Error(`--bundleMeta cannot override '${reserved}' — it is computed from the generated output`);
			}
		}
	}
