...
export class TypeDocPipeline {
  ...
  async createTypeDocApp(module: GirModule): Promise<TypeDocAppResult> {
    ...
    for (const child of result.project.children ?? []) {
      (child as unknown as { girNamespaceMetadata?: GirNamespaceMetadata }).girNamespaceMetadata ??=
        nsMeta;
    }
  ...
  private enrichMergedModuleMetadata(project: ProjectReflection): void {
    ...
    for (const child of project.children) {
      const enriched = child as DeclarationReflection & {
      ...
      enriched.girNamespaceMetadata = {
        ...
        logoUrl:
          existing.logoUrl ??
          meta.logoUrl ??
          (meta.iconFile ? `assets/library-icons/${meta.iconFile}` : undefined),
  ...
  private async bootstrapApp(
    ...
    const app = await Application.bootstrap(
      { ...this.sharedBootstrapOptions, ...options },
      readers,
    );
    ...
    for (const r of Object.values(project.reflections)) {
      ...
      for (const group of refl.groups) {
        ...
        const childrenToSplit =
          hasCategories && noneIdx >= 0
            ? group.categories?.[noneIdx].children
            : !hasCategories
              ? group.children
              : null;
  ...
  private mergeMetadataWithPackageJson(
    ...
    return {
      ...
      logoUrl:
        meta?.logoUrl ?? (meta?.iconFile ? `assets/library-icons/${meta.iconFile}` : undefined),
