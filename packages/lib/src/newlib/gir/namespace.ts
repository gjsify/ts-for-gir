...
import { ParsedGir, GirInfoAttrs, GirType } from "../../index.js";
import { isPrimitiveType } from "./util.js";

import { IntrospectedClass, IntrospectedInterface, IntrospectedRecord, IntrospectedBaseClass } from "./class.js";
...
export class IntrospectedNamespace {
    ...
    static fromXML(repo: ParsedGir, options: LoadOptions, registry: NSRegistry): IntrospectedNamespace {
        ...
        if (ns.record) {
            ns.record
                ?.filter(isIntrospectable)
                .map(record => IntrospectedRecord.fromXML(modName, building, options, record))
                .filter(importConflicts)
                .forEach(c => building.members.set(c.name, c));
        }
        ...
        if (ns.union) {
            ns.union
                ?.filter(isIntrospectable)
                .map(union => IntrospectedRecord.fromXML(modName, building, options, union))
                .filter(importConflicts)
                .forEach(c => building.members.set(c.name, c));
        }
        ...
        if (ns.interface) {
            ns.interface
                ?.map(inter => IntrospectedInterface.fromXML(modName, building, options, inter))
                .filter(importConflicts)
                .forEach(c => building.members.set(c.name, c));
        }
        ...
        try {
            ...
            if (
                major instanceof IntrospectedConstant &&
                minor instanceof IntrospectedConstant &&
                major.value &&
                minor.value
            ) {
