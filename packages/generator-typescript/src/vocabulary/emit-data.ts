...
export function emitVocabularyData(surface: WidgetVocabulary): string {
	...
	// The same join one case wider: the GType of a settable property whatever its type is.
	// `PROP_ENUMS` answers only where that type is an enum or bitfield, which covers 7.6% of the
	// rows and leaves a consumer that must infer a type FROM a property with nothing for the
	// other 92.4%.
	const propTypes = byGType.flatMap((decl) =>
		decl.props
			.filter((prop) => prop.gtype !== undefined)
			.map((prop) => `    '${decl.gtype}.${prop.girName}': '${prop.gtype}',`),
	);
...
export const PROP_TYPES = ${record(propTypes)};
