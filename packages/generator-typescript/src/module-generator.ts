...
export class ModuleGenerator extends FormatGenerator<string[]> {
  ...
  constructor(namespace: GirModule, config: OptionsGeneration, registry: NSRegistry) {
    ...
    if (this.config.reporter) {
      ...
      reporterService.registerReporter(
        `${ModuleGenerator.name}(${namespace.packageName})`,
        this.log,
      );
    }
  ...
  private buildDocContext(): GirDocContext {
    ...
    const docBaseUrl =
      DOC_BASE_URLS.get(`${ns.namespace}-${ns.version}`) ?? DOC_BASE_URLS.get(ns.namespace);
...
`;
  ...
  generateInterface(node: IntrospectedInterface): string[] {
    const isGObject = node.someParent(
    ...
    const hasVirtualMethods = node.members.some(
      (m) => m instanceof IntrospectedVirtualClassFunction,
    );

    const hasNamespace =
      isGObject || hasStaticFunctions || node.callbacks.length > 0 || hasVirtualMethods;
  ...
  generateProperty(tsProp: IntrospectedProperty, construct?: boolean, indentCount = 0) {
    ...
    else if (tsProp.readable && !tsProp.writable)
      propTags.push({ tagName: "read-only", paramName: "", text: "" });
    else if (tsProp.writable && !tsProp.readable)
      propTags.push({ tagName: "write-only", paramName: "", text: "" });
    ...
    const Type =
      type.resolve(this.namespace, this.options).rootPrint(this.namespace, this.options) || "any";
  ...
  generateField(tsProp: IntrospectedField, indentCount = 0) {
    ...
    desc.push(
      ...this.addGirDocComment(
        tsProp.doc,
        this.namespace.getTsDocMetadataTags(tsProp.metadata),
        indentCount,
      ),
    );
    ...
    if (text || tags.length) {
      ...
      for (const tag of tags) {
        const tagText = tag.text
          ? transformGirDocTagTextWithContext(tag.text, this._docContext)
          : "";
        if (tag.paramName) {
  ...
  generateParameter(
    tsParam: IntrospectedFunctionParameter,
    direction: GirDirection = GirDirection.In,
  ) {
  ...
  generateCallback(
    ...
    const callbackTags =
      tsCallback instanceof IntrospectedCallback &&
      !(tsCallback instanceof IntrospectedClassCallback)
        ? [
  ...
  generateConst(tsConst: IntrospectedConstant, indentCount = 0) {
    ...
    const typeStr = this.generateType(
      resolveDirectedType(tsConst.type, GirDirection.Out) ?? tsConst.type,
    );
  ...
  generateAlias(girAlias: IntrospectedAlias, indentCount = 0) {
    ...
    desc.push(
      ...this.addGirDocComment(
        girAlias.doc,
        [
          ...this.getGirTypeTags(girAlias),
          ...this.namespace.getTsDocMetadataTags(girAlias.metadata),
        ],
        indentCount,
      ),
    );
  ...
  generateConstructPropsInterface(
    ...
    const constructorPropMembers = filterConflicts(
      girClass.namespace,
      girClass,
      props,
      FilterBehavior.PRESERVE,
    )
      ...
      ...this.generateFunctions(
        filterFunctionConflict(
          ...
          [...girClass.members].filter(
            (member) => member instanceof IntrospectedStaticClassFunction,
          ),
          [],
        ),
        indentCount,
  ...
  generateClassConstructors(
    ...
    if (girClass.mainConstructor instanceof IntrospectedDirectAllocationConstructor)
      ...
      def.push(
        `${indent}constructor(properties?: Partial<${girClass.name}.ConstructorProps>, ...args: any[]);`,
      );
    }
    ...
    const hasInit =
      ...
      (!girClass.__ts__indexSignature ||
        girClass.__ts__indexSignature.includes("[key: string]: any"));
    if (hasInit) {
    ...
    for (const constructorFunction of filterFunctionConflict(
      girClass.parent,
      girClass,
      girClass.constructors,
      [],
    )) {
      if (def.length > 0) def.push("");
    ...
    def.push(
      ...this.generateFunctions(
        filterFunctionConflict(
          girClass.parent,
          girClass,
          [...girClass.members.values()].filter(
            (fn) => fn instanceof IntrospectedVirtualClassFunction,
          ),
          [],
        ),
        indentCount,
  ...
  private generateMethodSignature(method: IntrospectedVirtualClassFunction): string {
    const params = method.parameters
      .map((p) => `${p.name}:${p.type.print(this.namespace, this.config)}`)
      .join(",");
    const returnType = method.return().print(this.namespace, this.config);
    return `${method.name}(${params}):${returnType}`;
  }

  generateClassNamespaces(
    girClass: IntrospectedClass | IntrospectedRecord | IntrospectedInterface,
    indentCount = 0,
  ) {
    const def: string[] = [];
   ...
   * @param namespace
   */
  generateImplementationInterface(
    girClass: IntrospectedClass | IntrospectedRecord | IntrospectedInterface,
  ) {
    const def: string[] = [];
    ...
    if (girClass instanceof IntrospectedInterface) {
      def.push(
        ...this.addGirDocComment(
          girClass.doc,
          [
            ...this.getGirTypeTags(girClass),
            ...this.namespace.getTsDocMetadataTags(girClass.metadata),
          ],
          0,
        ),
      );
  ...
  generateClass(girClass: IntrospectedClass | IntrospectedRecord) {
    ...
    // START CLASS
    {
      ...
      const isOpaque =
        girClass instanceof IntrospectedRecord && girClass.isPrivate && !girClass.mainConstructor;
  ...
  async generateModuleDeclaration(node: GirModule): Promise<string | null> {
    try {
      ...
      if (!result) {
        this.log.reportGenerationFailure(
          node.namespace,
          new Error("Failed to generate module"),
          "Module Declaration",
        );
        return null;
      }
...
`;
      ...
      if (!result) {
        this.log.reportGenerationFailure(
          node.namespace,
          new Error("Failed to generate inline content"),
          "DTS Inline",
        );
        return null;
      }
