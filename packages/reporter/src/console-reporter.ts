...
export class ConsoleReporter extends ReporterBase {
  ...
  public reportTypeResolutionError(
    typeName: string,
    namespace: string,
    message: string,
    details?: string,
  ...
  public reportParsingFailure(
    ...
    this.addProblem(
      ProblemSeverity.ERROR,
      ProblemCategory.PARSING_FAILURE,
      message,
      details,
      itemName,
      namespace,
  ...
  public reportGenerationFailure(namespace: string, error: Error | string, context?: string): void {
    ...
    this.addProblem(
      ProblemSeverity.ERROR,
      ProblemCategory.GENERATION_FAILURE,
      message,
      details,
      undefined,
      namespace,
  ...
  public printSummary(): void {
    ...
    if (statistics.totalProblems > 0) {
      ...
      if (statistics.typeStatistics.commonTypeConflicts.length > 0) {
        console.log(`\n⚔️  Type Conflicts:`);
        statistics.typeStatistics.commonTypeConflicts.forEach(
          ({ conflictType, count, examples }) => {
      ...
      if (statistics.typeStatistics.problematicNamespaces.length > 0) {
        ...
        statistics.typeStatistics.problematicNamespaces
          .slice(0, 5)
          .forEach(({ namespace, problems, types }) => {
