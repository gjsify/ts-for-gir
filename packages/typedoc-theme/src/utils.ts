...
export function hasSignalTag(refl: Reflection): boolean {
  ...
  if (signatures) {
    return signatures.some(
      (sig) => sig.comment?.modifierTags.has("@signal" as `@${string}`) ?? false,
    );
  }
  return false;
}
 ...
 */
export function girMemberBadgesFromReflection(
  refl: Reflection,
): Array<{ label: string; cssModifier: string }> {
  ...
  const isVirtual =
    hasVirtualTag(refl) || gir?.girKind === "virtual-method" || refl.name.startsWith("vfunc_");
  ...
  const constructOnly =
    hasModifierTag(refl, "@construct-only") || gir?.propertyMetadata?.constructOnly;
  const readOnly =
    hasModifierTag(refl, "@read-only") ||
    (gir?.propertyMetadata?.readable && !gir.propertyMetadata.writable);
  const writeOnly =
    hasModifierTag(refl, "@write-only") ||
    (gir?.propertyMetadata?.writable && !gir.propertyMetadata.readable);
  ...
  if (refl.isDeprecated()) {
    ...
    badges.push({
      label: version ? `Deprecated ${version}` : "Deprecated",
      cssModifier: "deprecated",
    });
  }
 ...
 */
export function findCompanionNamespace(
...
export function isCompanionNamespace(nsRefl: DeclarationReflection): boolean {
  ...
  return siblings.some(
    (child) =>
      child !== nsRefl && child.kindOf(COMPANION_OWNER_KINDS) && child.name === nsRefl.name,
  );
}
