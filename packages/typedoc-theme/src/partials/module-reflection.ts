...
export function renderCategorizedProject(
  ...
  for (const child of children) {
    const nsMeta = (child as unknown as { girNamespaceMetadata?: GirNamespaceMetadata })
  ...
  return JSX.createElement(
    ...
    mod.hasComment(
      context.options.getValue("notRenderedTags") as unknown as readonly `@${string}`[],
    ) &&
      JSX.createElement(
        "section",
...
export function giDocgenModuleReflection(
  ...
  if (mod.isDeclaration() && isCompanionNamespace(mod as DeclarationReflection)) {
    ...
    const siblings =
      parent && "children" in parent ? (parent as DeclarationReflection).children : undefined;
...
export function giDocgenModuleMemberSummary(
  ...
  context.page.pageHeadings.push({
    ...
    kind:
      member instanceof ReferenceReflection ? member.getTargetReflectionDeep().kind : member.kind,
  ...
  if (member instanceof ReferenceReflection) {
    ...
    name = JSX.createElement(
      ...
      JSX.createElement(
        "span",
        { class: classNames({ deprecated: member.isDeprecated() }) },
        member.name,
      ),
      ...
      JSX.createElement(
        ...
        JSX.createElement(
          ...
          JSX.createElement("use", {
            href: `${context.relativeURL("assets/icons.svg", true)}#icon-anchor`,
          }),
        ),
      ),
    );
  ...
  const nsMeta = (member as unknown as { girNamespaceMetadata?: GirNamespaceMetadata })
    .girNamespaceMetadata;
  const commentSummary = context.commentShortSummary(member);
  const description =
    commentSummary ||
    (nsMeta?.description ? JSX.createElement("p", null, nsMeta.description) : null);
